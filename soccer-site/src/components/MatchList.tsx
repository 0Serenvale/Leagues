        ...
        return (
          <div key={match.id} className="flex justify-between items-center py-2 px-3 border-b border-gray-100 bg-gray-50 hover:bg-white transition-colors text-sm rounded">
            ...
            <div className="w-1/3 flex justify-end gap-2 items-center font-semibold text-gray-800">
               <span className="truncate" title={homeTeam?.name}>{homeTeam?.name?.substring(0,10)}</span>
            ...
            <div className={`w-1/3 flex justify-center items-center gap-1 font-bold rounded px-2 py-1 ${isLive ? 'bg-red-100 text-red-700 animate-pulse' : 'bg-gray-200 text-gray-800'}`}>
              <span className="text-base">{match.homeScore ?? '-'}</span>
              <span className="text-xs text-gray-500 mx-1">:</span>
              <span className="text-base">{match.awayScore ?? '-'}</span>
            </div>
            ...
            <div className="w-1/3 flex justify-start gap-2 items-center font-semibold text-gray-800">
               ...
               <span className="truncate" title={awayTeam?.name}>{awayTeam?.name?.substring(0,10)}</span>
            </div>
